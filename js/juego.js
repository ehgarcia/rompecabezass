...
var instrucciones = [
  "Mueve la pieza vacia con las flecas del teclado",
  "completa el rompezabezas en el menor numero de movimientos posibles"
];
...
function mostrarInstrucciones(instrucciones) {
  for (var i = 0; i < instrucciones.length; i += 1) {
...
function chequearSiGano() {
  var check = [];
  for (var i = 0; i < grilla.length; i += 1) {
    for (var j = 0; j < grilla.length; j += 1) {
      check.push(grilla[i][j]);
    }
  }
  if (
    check[0] == 1 &&
...
*/
function intercambiarPosicionesGrilla(row1, column1, row2, column2) {
  var value1 = grilla[row1][column1];
  grilla[row1][column1] = grilla[row2][column2];
  grilla[row2][column2] = value1;
}

// Actualiza la posición de la pieza vacía
function actualizarPosicionVacia(newRow, newColumn) {
  filaVacia = newRow;
  columnaVacia = newColumn;
}

// Para chequear si la posicón está dentro de la grilla.
function posicionValida(row, column) {
